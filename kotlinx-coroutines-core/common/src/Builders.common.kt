 ...
 * as `launch`, but allows to specify `catch` and `finally` blocks before building a coroutine with
 * [build][CoroutineBuilder.build]. In this way, the proper non-cancellable context is already established
 * for `catch` and `finally` blocks:
 *
 * ```
 ...
 * ```
 *
 * Note, that for complex cases semantics of [catch][CoroutineBuilder.catch] and [finally][CoroutineBuilder.finally]
 * blocks is different from one `try/catch/finally` expression, as they are always executed top-to-bottom,
 * with each subsequently installed handler corresponding to a separate `try { ... }`.
...
private class LaunchBuilder(
    ...
    override fun updateBlock(block: suspend CoroutineScope.() -> Unit): CoroutineBuilder<Job, CoroutineScope, Unit> =
